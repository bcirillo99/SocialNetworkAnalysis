...
from task3.idm import generalize_diffusion_mechanism
import matplotlib
...
def input_data():
    ...
    auctions["IDM"] = [True, True, generalize_diffusion_mechanism]
...
list_auction = ["VCG","MUDAR","MUDAN"] # Different Auctions
dict_armset = {"pagerank": arms_set_page_rank, "degree": arms_set_degree, "voterank": arms_set_vote_rank, "shapley_degree": arms_set_shapley_degree, "shapley_threshold": arms_set_shapley_threshold, "shapley_closeness": arms_set_shapley_closeness}
...
for auction in list_auction:
    ...
    for key in dict_armset.keys():
        ...
        for k in listk:
            ...
            dict_results["Bandit"].append("UCB_mean")
            dict_results["Auction"].append(auction)
            dict_results["Time (s)"].append(exe_time_ucb)
            dict_results["T"].append(T)
            dict_results["k"].append(k)
            ...
            snm_ucb=SocNetMec_UCB_mean(G=G, T=T, k=k, auctions=auctions, arms_set=arms_set, auction=auction, gaussian_dist=prior)
            ucb_revenue = 0
            opt_ucb_reward = 0
            regrets_ucb = {}
            ...
            dict_results["Bandit"].append("UCB_mean_prior")
            dict_results["Auction"].append(auction)
            dict_results["Time (s)"].append(exe_time_ucb)
            dict_results["T"].append(T)
            dict_results["k"].append(k)
            ...
            df1.to_csv("final_results_idm/final_"+auction+"_setridotto.csv", index=False)
