...
def input_data(T=20000, path_net=None):
    n = 100
    r = 2.7
    kw = 1
    q = 4
...
parser.add_argument('--path', help='numero di processi', type=str, default=None)
args = parser.parse_args()


if args.path is not None:
    ext = args.path[-6:].replace("txt","csv")
    G, k, T, val, p, arms_set, auctions = input_data(T=args.T, path_net=args.path)
else:
    ext = ".csv"
...
list_auction = ["GIDM","MUDAN"] # Different Auctions
dict_armset = {"pagerank": arms_set_page_rank, "degree": arms_set_degree, "voterank": arms_set_vote_rank, "shapley_degree": arms_set_shapley_degree}
dict_distributions = {"pagerank": gaussian_dist_pagerank, "degree": gaussian_dist_degree, "voterank": gaussian_dist_vote_rank, "shapley_degree": gaussian_dist_shapley_degree}
...
for auction in list_auction:
    ...
    for key in dict_armset.keys():
        ...
        for k in listk:
            ...
            path = "results_armset_prior_2/"+str(T)
        
            if not os.path.exists(path):
                os.makedirs(path)
            df1.to_csv(path+"/"+auction+ext, index=False)
