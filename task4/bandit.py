...
class Bernoulli_Environment:
    ...
    def __init__(self, means_dict):
        ...
        ----------
        means_dict: dict
            dictionary whose keys are the arms and whose values the respective mean
        """
    ...
    def receive_reward(self, arm):
        ...
        ----------
        arm:
            the arm that the learner want to use
...
class Social_Environment:
    ...
    def __is_alive(self, u,v):
        ...
        """
        r = random.random()
        if r <= self.__prob_dict[(u,v)]: #with probability p(u,v)
            return True
        else:
            return False
