import os
import pandas as pd
for ext in [".csv","_2.csv"]:
    for t in ["20000","40000","60000"]:
        #df_mudan = pd.read_csv("results_armset_prior_2/"+t+"/MUDAN"+ext)
        df_mudar = pd.read_csv("results_armset_prior_2/"+t+"/MUDAN"+ext)
        #df_vcg = pd.read_csv("final_results/20000/VCG"+ext)
        df_gidm = pd.read_csv("results_armset_prior_2/"+t+"/GIDM"+ext)
        ...
        for k in listk:
            ...
            results.append(pd.concat([df_mudar_k,df_gidm_k]))
            #results.append(pd.concat([df_gidm_k]))

        final_results = pd.concat(results).reset_index(drop=True)
        #print(final_results)
        path = "results_armset_prior_2/"+t+"/results/"
                
        if not os.path.exists(path):
            os.makedirs(path)
        final_results.to_csv(path+"basic_auction_best_results"+ext)
        ...
        final_results = pd.concat(results).reset_index(drop=True)
        print(final_results)
        #final_results.to_csv(path+"best_results"+ext,columns=["Bandit", "T",  "k", "Revenue", "armset"])
        final_results.to_csv(path+"best_results"+ext,columns=["Bandit", "Auction", "Revenue", "armset"])